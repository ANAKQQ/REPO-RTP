...
var inAnimation = 0;
function moveTabs(parDir) {
    var win_width = $(window).width();
    var max_item = 4;
    var con_width = $(".tab-nav").width();
    var max_width = (con_width - 80 - 15) / 4;
    var currPage = parseInt($(".tab-nav").attr("cp"));
    var totalPage = parseInt($(".tab-nav").attr("tp"));
    var moveLeft = 0;
    if (win_width < 980) {
        //max_width = 150;
        currPage = parseInt($(".tab-nav").attr("mcp"));
        max_width = (con_width - 80 - 10) / 3;
        max_item = 3;
        totalPage = parseInt($(".tab-nav").attr("mtp"));
    }
    if (inAnimation == 0) {
        if (parDir == "prev") {
            if (currPage > 1) {
                moveLeft = max_item * max_width;
                inAnimation = 1;
                $(".tab-nav .prov").each(function() {
                    var pos = $(this).position();
                    $(this).animate({
                        "left": (pos.left + moveLeft + (max_item * 5)) + "px"
                    });
                ...
                currPage -= 1;
                if (win_width < 980) {
                    $(".tab-nav").attr("mcp", currPage);
                } else {
                    $(".tab-nav").attr("cp", currPage);
                }
            }
        } else if (parDir == "next") {
            if (currPage < totalPage) {
                moveLeft = max_item * max_width * (-1);
...
if ($(".tab-nav .prov").length) {
    var cp = parseInt($(".cp").html());
    var mcp = parseInt($(".mcp").html());
    if (!isNaN(cp)) {
        $(".tab-nav").attr("cp", cp);
    }
    if (!isNaN(mcp)) {
        $(".tab-nav").attr("mcp", mcp);
    }
    var cnt = 0;
    var pos_left = 0;
    var win_width = $(window).width();
    ...
    $(".tab-nav .prov").each(function() {
        pos_left = (cnt * max_width) + 40 + (cnt * 5);
        $(this).css({
            "width": max_width + "px",
            "left": pos_left + "px"
        });
        ...
        if (win_width < 980) {
            moveLeft = (mcp - 1) * max_item * max_width * (-1);
        } else {
            moveLeft = (cp - 1) * max_item * max_width * (-1);
        }
        moveLeft = (currPage - 1) * max_item * max_width * (-1);
        $(".tab-nav .prov").each(function() {
            var pos = $(this).position();
            $(this).animate({
                "left": (pos.left + moveLeft - ((max_item * 5) * (currPage - 1))) + "px"
            });
        });
    });
}

if ($(".tab-content .c-patt").length) {
    var cPatternHeight = 0;
    $(".tab-content .c-patt").each(function() {
        if ($(this).outerHeight() > cPatternHeight) {
            cPatternHeight = $(this).outerHeight();
        }
    }).promise().done(function() {
        $(".tab-content .c-patt").css({
            "height": cPatternHeight + "px"
        });
    });
}
var progressive_jackpot = document.getElementById('progressive_jackpot');
var startNominal = 22000000000;
var endNominal = 5000000000000;
var randomNominal = 0;

function updateprogressive_jackpot() {
  randomNominal = Math.floor(Math.random() * (1000000 - 50000 + 1)) + 50000;
  startNominal += randomNominal;
  if (startNominal > endNominal) {
    startNominal = endNominal;
  }
  progressive_jackpot.innerHTML = 'IDR ' + startNominal.toLocaleString();
}
updateprogressive_jackpot();
setInterval(updateprogressive_jackpot, 500);
